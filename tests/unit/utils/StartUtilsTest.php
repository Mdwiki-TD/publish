...
class StartUtilsTest extends TestCase
{
    ...
    {
        $this->assertSame('John Doe', \Publish\StartUtils\formatUser('John_Doe'));
    }

    public function testFormatUserMapsSpecialAlias(): void
    {
        $this->assertSame('Mr. Ibrahem', \Publish\StartUtils\formatUser('Mr. Ibrahem 1'));
        $this->assertSame('Mr. Ibrahem', \Publish\StartUtils\formatUser('Admin'));
    }

    public function testFormatUserPassesThroughRegularUser(): void
    {
        $this->assertSame('Regular User', \Publish\StartUtils\formatUser('Regular_User'));
    }

    public function testFormatTitleNormalisesUnderscores(): void
    {
        $this->assertSame('My Article', \Publish\StartUtils\formatTitle('My_Article'));
    }

    public function testFormatTitleFixesMrIbrahem1Prefix(): void
    {
        $result = \Publish\StartUtils\formatTitle('Mr. Ibrahem 1/SomeArticle');
        $this->assertSame('Mr. Ibrahem/SomeArticle', $result);
    }

    public function testDetermineHashtagDefaultValue(): void
    {
        $hashtag = \Publish\StartUtils\determineHashtag('SomeArticle', 'RegularUser');
        $this->assertSame('#mdwikicx', $hashtag);
    }

    public function testDetermineHashtagEmptyForMrIbrahemOwnPage(): void
    {
        $hashtag = \Publish\StartUtils\determineHashtag('Mr. Ibrahem/SomePage', 'Mr. Ibrahem');
        $this->assertSame('', $hashtag);
    }

    public function testDetermineHashtagKeptWhenDifferentUser(): void
    {
        $hashtag = \Publish\StartUtils\determineHashtag('Mr. Ibrahem/SomePage', 'OtherUser');
        $this->assertSame('#mdwikicx', $hashtag);
    }
